...
export function buildSimulationPayload({ rank, diag, hometax, equipped, products = [] }) {
  ...
  // 백엔드 검증 규칙: 잔액이 0이면 상환액도 0이어야 한다 (KB 연동이 상환액만 채운 경우 정규화)
  const existingMonthlyPayment = existingDebtTotal > 0
    ? Math.max(0, Number(diag.existingMonthlyPaymentMan || 0) * 10_000) : 0;
