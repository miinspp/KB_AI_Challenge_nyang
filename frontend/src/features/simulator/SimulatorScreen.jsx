  ...
  return (
    <div className="scr" style={{ padding: '0 0 130px', gap: 0 }}>
      <div style={{ padding: '2px 22px 12px' }}>
        <h2 style={{ fontSize: 19, fontWeight: 900, color: 'var(--ink)', letterSpacing: -.4 }}>상품을 장착해 보세요</h2>
        <p style={{ marginTop: 4, fontSize: 12.5, color: 'var(--muted)' }}>아래 상품을 탭하면 슬롯에 장착되고, 가게 지표가 바뀌어요.</p>
      </div>

      <div style={{
        margin: '0 22px', position: 'relative', height: 230, overflow: 'hidden',
        background: 'linear-gradient(180deg,#FFF3D2 0%,var(--canvas) 68%,#E9F2DB 86%,#D8E8C2 100%)',
        ...
        <div style={{
          ...
          background: 'rgba(255,255,255,.72)', border: '1.5px solid var(--border)', borderRadius: 16,
          padding: 10, display: 'flex', flexDirection: 'column', gap: 8, backdropFilter: 'blur(3px)',
        }}>
          <span style={{ fontSize: 10.5, fontWeight: 800, color: 'var(--muted)', letterSpacing: .3 }}>장착 상품 {eq.length}개</span>
          <div style={{ flex: 1, minHeight: 0, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 7 }}>
            {eq.length > 0 ? eq.map((p) => (
              <button key={p.id} onClick={() => toggle(p.id)} title={`${p.short} 해제`} style={{
                ...
                <span style={{ flex: 1, fontSize: 10.5, fontWeight: 800, lineHeight: 1.3, color: 'var(--ink)' }}>{p.short || p.name}</span>
              </button>
            )) : (
              <div style={{ flex: 1, borderRadius: 12, border: '1.5px dashed var(--border-deep)', background: 'rgba(255,255,255,.5)', display: 'grid', placeItems: 'center', padding: 10, textAlign: 'center' }}>
                <span style={{ fontSize: 10.5, fontWeight: 800, color: 'var(--muted-faint)', lineHeight: 1.45 }}>아래에서 상품을 선택해 주세요</span>
              </div>
            )}
          </div>
        </div>
      ...
      <div className="card" style={{ margin: '12px 22px 0', padding: '16px 14px', display: 'flex', flexDirection: 'column', gap: 11 }}>
        ...
        {!loading && simRows.map((m) => (
          <div key={m.name} style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
            ...
            <span style={{ fontSize: 12, color: 'var(--muted-faint)' }}>→</span>
            <span style={{ fontSize: 14, fontWeight: 900, color: 'var(--ink)', whiteSpace: 'nowrap' }}>{m.after}</span>
      ...
      <div className="card" style={{ margin: '12px 22px 0', padding: '15px 16px 16px', display: 'flex', flexDirection: 'column', gap: 14 }}>
        <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10 }}>
          ...
          <span style={{
            ...
            color: riskTone === 'base' ? 'var(--muted)' : riskTone === 'good' ? 'var(--green-text)' : 'var(--danger)',
            background: riskTone === 'base' ? '#F5EFE3' : riskTone === 'good' ? 'var(--green-bg)' : 'var(--danger-bg)',
        ...
        <div>
          <div style={{ display: 'flex', alignItems: 'baseline', gap: 7 }}>
            <p style={{ fontSize: 13, fontWeight: 900, color: 'var(--ink)' }}>{current.title}</p>
            <p style={{ fontSize: 11, fontWeight: 800, color: current.inverse ? (current.after <= current.before ? 'var(--green-text)' : 'var(--danger)') : (current.after >= current.before ? 'var(--green-text)' : 'var(--danger)') }}>
              {current.before}→{current.after}{current.unit}
            </p>
          </div>
          <p style={{ marginTop: 4, fontSize: 11.5, color: 'var(--muted)', lineHeight: 1.55 }}>{current.lead}</p>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 7 }}>
          {current.facts.map((f) => (
            <div key={f.k} style={{ borderTop: '1.5px solid var(--border)', paddingTop: 8, minWidth: 0 }}>
              <p style={{ fontSize: 10.5, fontWeight: 800, color: 'var(--muted-mid)' }}>{f.k}</p>
              <p style={{ marginTop: 3, fontSize: 12, fontWeight: 900, color: 'var(--ink)', lineHeight: 1.25 }}>{f.v}</p>
            </div>
          ))}
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
          {detail.summary.map((s) => (
            <div key={s.label} style={{ background: 'var(--warm)', borderRadius: 12, padding: '10px 11px' }}>
              <p style={{ fontSize: 10.5, fontWeight: 800, color: 'var(--muted-mid)' }}>{s.label}</p>
              <div style={{ marginTop: 4, display: 'flex', alignItems: 'baseline', gap: 5 }}>
                <span style={{ fontSize: 13, fontWeight: 900, color: 'var(--ink)' }}>{s.value}</span>
                <span style={{ fontSize: 10.5, fontWeight: 900, color: s.good ? 'var(--green-text)' : 'var(--danger)' }}>{s.delta}</span>
              </div>
            </div>
          ))}
        </div>
            ...
            <div key={c.id} style={{ display: 'flex', gap: 9, alignItems: 'flex-start', paddingTop: 8, borderTop: '1.5px solid var(--border)' }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: 'flex', gap: 8, alignItems: 'baseline' }}>
                  <p style={{ flex: 1, fontSize: 12, fontWeight: 900, color: 'var(--ink)' }}>{c.name}</p>
                ...
                <p style={{ marginTop: 3, fontSize: 10.8, color: 'var(--muted-mid)', lineHeight: 1.45 }}>{c.note}</p>
              </div>
            </div>
          )) : (
      ...
      <div style={{ marginTop: 14, padding: '0 22px' }}>
        <p style={{ fontSize: 12.5, fontWeight: 800, color: 'var(--muted)', marginBottom: 9 }}>
          보유 아이템 <span style={{ fontWeight: 500, color: 'var(--muted-faint)' }}>· 탭하여 장착/해제</span>
        </p>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
          {products.map((p) => {
            ...
            return (
              <button key={p.id} onClick={() => toggle(p.id)} style={{
                ...
                <span style={{ flex: 1, fontSize: 11.5, fontWeight: 800, color: 'var(--ink)', lineHeight: 1.35 }}>{p.short || p.name}</span>
                <span style={{ flex: 'none', fontSize: 13, fontWeight: 900, color: on ? 'var(--gold-link)' : '#D8CDBB' }}>{on ? '✓' : '+'}</span>
              </button>
            );
          })}
        </div>
...
function MiniCompareChart({ points, inverse, unit }) {
  ...
  // SVG stroke '속성'에 그대로 들어가므로 리터럴이어야 한다(속성은 var() 를 해석하지 못함).
  const afterColor = inverse ? '#8DBB6C' : '#FFBC00';

  return (
    <div style={{ background: 'var(--canvas)', border: '1.5px solid var(--border)', borderRadius: 14, padding: '11px 10px 9px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 9 }}>
        <p style={{ fontSize: 10.5, fontWeight: 900, color: 'var(--muted-mid)' }}>12개월 예측</p>
        <div style={{ display: 'flex', gap: 9, fontSize: 10.5, fontWeight: 800, color: 'var(--muted-mid)' }}>
      ...
      <div style={{ position: 'relative', height: 108, paddingLeft: 28 }}>
        <span style={{ position: 'absolute', left: 0, top: 1, fontSize: 8.5, fontWeight: 800, color: 'var(--muted-soft)' }}>{Math.round(max)}{unit}</span>
        <span style={{ position: 'absolute', left: 0, bottom: 18, fontSize: 8.5, fontWeight: 800, color: 'var(--muted-soft)' }}>{Math.round(min)}{unit}</span>
        ...
        <div style={{ position: 'absolute', left: 28, right: 0, bottom: 0, display: 'flex', justifyContent: 'space-between' }}>
          {points.map((p, i) => <span key={p.label} style={{ fontSize: 8.5, fontWeight: 800, color: i % 3 === 0 || i === points.length - 1 ? 'var(--muted-soft)' : 'transparent' }}>{p.label}월</span>)}
        </div>
      </div>
    </div>
  );
...
function RiskGauge({ before, after }) {
  ...
  const color = afterValue < 20 ? 'var(--green-text)' : afterValue < 40 ? 'var(--gold-link)' : 'var(--danger)';

  return (
    <div style={{ background: 'var(--warm)', borderRadius: 12, padding: '11px 12px 10px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline' }}>
        <p style={{ fontSize: 11, fontWeight: 900, color: 'var(--muted)' }}>현금 부족 가능성</p>
      ...
      <div style={{ position: 'relative', height: 13, marginTop: 10, borderRadius: 7, background: 'linear-gradient(90deg,#A8D284 0 20%,#FFD66B 20% 40%,#F0968C 40% 100%)' }}>
        <span title={`기준 ${beforeValue}%`} style={{ position: 'absolute', left: `${beforeValue}%`, top: -4, width: 2, height: 21, background: 'var(--ink-soft)', transform: 'translateX(-1px)' }} />
      ...
      <div style={{ marginTop: 5, display: 'flex', justifyContent: 'space-between', fontSize: 9.5, fontWeight: 800, color: 'var(--muted-mid)' }}>
        <span>0% 낮음</span><span>20% 주의</span><span>40% 이상 높음</span>
      </div>
      <p style={{ marginTop: 8, fontSize: 10.5, color: 'var(--muted)', lineHeight: 1.45 }}>
